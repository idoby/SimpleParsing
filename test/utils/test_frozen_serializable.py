...
@dataclass(frozen=True)
class Parent(FrozenSerializable):
    name: str = "Consuela"
...
@dataclass(frozen=True)
class ParentWithOptionalChildrenWithFriends(FrozenSerializable):
    name: str = "Consuela"
...
@dataclass(frozen=True)
class Base(FrozenSerializable, decode_into_subclasses=True):
    name: str = "bob"
...
@dataclass(frozen=True)
class Container(FrozenSerializable):
...
def test_forward_ref_dict(silent):
    @dataclass(frozen=True)
    class LossWithDict(FrozenSerializable):
        name: str = ""
        total: float = 0.
...
def test_forward_ref_list(silent):
    ...
    @dataclass(frozen=True)
    class JLossWithList(FrozenSerializable):
        name: str = ""
        total: float = 0.
...
def test_forward_ref_attribute():
    @dataclass(frozen=True)
    class LossWithAttr(FrozenSerializable):
        name: str = ""
        total: float = 0.
...
def test_nested_list():
    @dataclass(frozen=True)
    class Kitten(FrozenSerializable):
        name: str = "Meow"

    @dataclass(frozen=True)
    class Cat(FrozenSerializable):
        name: str = "bob"
        age: int = 12
...
def test_from_dict_raises_error_on_failure():
    ...
    @dataclass(frozen=True)
    class Something(FrozenSerializable):
        name: str
        age: int = 0
...
def test_custom_encoding_fn():
    @dataclass(frozen=True)
    class Person(FrozenSerializable):
...
def test_set_field():
    ...
    class Person(FrozenSerializable, Hashable):
        name: str = "Bob"
        age: int = 0
    ...
    @dataclass(frozen=True)
    class Group(FrozenSerializable):
...
def test_used_as_dict_key():
    ...
    @dataclass(frozen=True)
    class Leaderboard(FrozenSerializable):
